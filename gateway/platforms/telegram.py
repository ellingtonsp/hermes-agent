...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def stop_typing(self, chat_id: str) -> None:
        """Telegram typing indicator can't be cancelled server-side (auto-expires
        in ~5s), but we drop the per-chat cooldown and rely on
        ``_typing_paused`` (added by the base ``_stop_typing_refresh``) so any
        straggler refresh from the keep-typing loop is a no-op.  Combined with
        the 2.0s cancel timeout in ``_stop_typing_refresh``, this ensures the
        typing bubble disappears within ~5s of the final message landing."""
        try:
            self._telegram_typing_cooldown_until.pop(str(chat_id), None)
        except Exception:
            pass
