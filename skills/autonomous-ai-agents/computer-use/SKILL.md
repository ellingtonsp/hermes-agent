...
For a real human-only checkpoint or an explicit handoff test, first stop desktop input and save the task/checkpoint at `$HERMES_HOME/desktop-tasks/<task>.json`. When a user says `resume <task>`, read that exact checkpoint even if the task began on another chat surface; do not assume missing current-chat context means there is no task. Run this skill's `scripts/telegram_handoff.py` with `--chat-id`, `--url` (the configured tailnet HTTPS launcher), `--reason` (precise non-secret instruction), and `--task` (short ID). Add `--thread-id` only for the originating Telegram topic. Use `--dry-run` to inspect the payload. It reads only the active profile's Telegram credential and sends one labeled URL button, no raw transport URL in the message text.
